...
function getGames(sport, onLoad, onError) {
    ...
    req.onload = function (e) {
        if (req.readyState == 4) {
            // 200 - HTTP OK
            if (req.status == 200) {
                const sportsData = JSON.parse(req.responseText); //<-right here
                const events = sportsData.events; // after exploring a bit I found that games are actually an array inside this events object
